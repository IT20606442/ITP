...
import { fetchAllVets, createVet, updateVet } from "actions/VetAction";
...
const validationSchemaVetCreate = yup.object({
  First_Name: yup
...
const validationSchemaVetEdit = yup.object({
  First_Name: yup
    .string("Enter your first name")
    .max(255, "First name should be of maximum 255 characters length"),
  Last_Name: yup
...
function VetForm() {
  ...
  const selectVet = (state) => {
    const itemArray = state.vet.vets?.filter((item) => (vetId ? item._id === vetId : false));
    return itemArray && itemArray[0] ? itemArray[0] : null;
  };

  const vet = useSelector(selectVet);
  ...
  useEffect(() => {
    if (vet && vet?._id) {
      setFile(`http://localhost:8070/${vet.profileImage}`);
      setFileName(vet.profileImage);
    }
    return () => {
      setFile("");
      setFileName("");
  ...
  const formik = useFormik({
    initialValues: {
      ...
      Available_Day: vet?.Available_Day || "",
      Available_Date: vet?.Available_Date || "",
      Available_STime: vet?.Available_STime || "",
      Available_ETime: vet?.Available_ETime || "",
      UserType: vet?.UserType || "vet",
    },
    validationSchema: vet && vet._id ? validationSchemaVetEdit : validationSchemaVetCreate,
    onSubmit: (values, onSubmitProps) => {
      delete values.confirmPassword;
      if (values._id) {
        dispatch(updateVet(values, history));
      } else {
        dispatch(createVet(values, history));
      }
